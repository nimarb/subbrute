...
def print_target(
    ...
    if not print_data:
        dupe_filter = {}
    for result in run(
        pbar, target, query_type, subdomains, resolve_list, process_count
    ):
...
def run(
    ...
    verify_nameservers_proc = verify_nameservers(
        target, query_type, resolve_q, resolve_list, is_authoritative
    )
    ...
    if list_len < process_count:
        ...
        if process_count <= 0:
            process_count = 1
        trace(
            "Too few resolvers:", list_len, " process_count reduced to:", process_count
        )
    worker_list = []
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if (
                        type(e) == Queue.Empty
                        or str(type(e)) == "<class 'queue.Empty'>"
                    ):
                        x = prog_c.value()
...
def main():
    ...
    parser.add_option(
        ...
        type="string",
        help="(optional) A list of subdomains, accepts a single file, or a directory of files. default = 'names.txt'",
    )
    parser.add_option(
        "-r",
        "--resolvers",
